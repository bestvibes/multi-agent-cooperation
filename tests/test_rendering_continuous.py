...
class TestRenderingContinuous(unittest.TestCase):
    def straight_to_target(self, state_space, start_state):
        action_space = 1
        obstacles = None
    ...
    @unittest.skip("this doesn't work and crashes due to matplotlib not playing well with MacOS and pytest")
    def test_render(self, show=False):
        start_state = (((3,4),(0,0)),((15,20),(0,0)))
        state_space = (20, 20)
        state_sequence, reward_sequence = self.straight_to_target(state_space, start_state)
        render = Render_continuous(state_space, None)
        return render(state_sequence, reward_sequence, show=show)

if __name__ == '__main__':
    anim = TestRenderingContinuous().test_render(show=True)