...
class TestReplayMemoryPusher(unittest.TestCase):
    def test_push_to_capacity(self):
        replay_memory = []
        capacity = 5
        pusher = src.replay_memory.ReplayMemoryPusher(capacity)
        for i in range(capacity):
            replay_memory = pusher(replay_memory, i)

        self.assertEqual([0, 1, 2, 3, 4], replay_memory)
    ...
    def test_push_over_capacity(self):
        ...
        for i in range(capacity):
            replay_memory = pusher(replay_memory, i)
            self.assertTrue(i in replay_memory and len(replay_memory) == capacity)
